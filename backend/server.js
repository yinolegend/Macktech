...
function getHandbookFilesWithVisibility() {
...
app.get('/api/handbook', async (req, res) => {
  try {
    ...
    const list = getHandbookFilesWithVisibility();
...
app.post('/api/handbook', authMiddleware, (req, res) => {
  uploadHandbook.single('pdf')(req, res, (err) => {
    ...
    try {
      ...
      res.json({ ok: true, filename: req.file.filename, url: `/pdf-handbook/${encodeURIComponent(req.file.filename)}` });
    } catch (err) {
...
app.patch('/api/handbook/:filename', authMiddleware, (req, res) => {
  try {
    ...
    res.json({ ok: true, filename, hidden });
  } catch (err) {
